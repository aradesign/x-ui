...
install_packages() {
  ...
  apt update -y
  apt install -y git curl ca-certificates gnupg lsb-release build-essential
}
...
clone_repo() {
  if [ -z "${REPO_URL}" ]; then
    read -r -p "Repo URL (git): " REPO_URL
  fi
...
build_app() {
  ...
  /usr/local/go/bin/go build -ldflags "-w -s" -o x-ui main.go
}
...
main() {
  ...
  if [ -n "${DOMAIN}" ]; then
    ...
    log "Open: https://<server-ip>:2053 (default port)"
  fi
}
